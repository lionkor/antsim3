...
public:
    PlanetComponent(double mass)
        ...
        m_vel      = { Random::random_real(-50.0, 50.0), Random::random_real(-50.0, 50.0) };
    }
    ...
    virtual void on_update() override {
        ...
        for (auto& planet_ptr : s_planets) {
            ...
            std::clamp(r_squared, 0.1, std::numeric_limits<double>::max());
        ...
        auto color     = sf::Color(255, 255, 255, std::lerp(64, 255, m_mass / 5000.0));
...
static void init(Application& app) {
    ...
    sun.add_component(new PlanetComponent(5000));

    for (int i = 0; i < 10; ++i) {
        world.add_entity(new Entity({ window.width<double>() / 2 + Random::random(-1000, 1000), window.height<double>() / 2 + Random::random(-1000, 1000) }))
            .add_component(new PlanetComponent(Random::random(5, 200)));
    }
}
